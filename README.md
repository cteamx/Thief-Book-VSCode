...
`Alt+M` 老板键 。 随机显示不同语言的 Hello World ，随时可用，同时停止自动播放
...
`Alt+/` 自动播放 。 开启/关闭自动翻页，间隔时间可在设置中修改
...
`Alt+M` Boss key. Randomly displays Hello World in different languages, also stops auto play
...
`Alt+/` Auto play . Toggle auto page turning, interval can be changed in settings
